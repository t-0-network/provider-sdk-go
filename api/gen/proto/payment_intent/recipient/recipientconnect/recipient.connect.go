...
import (
	...
	context "context"
	errors "errors"
	recipient "github.com/t-0-network/provider-sdk-go/api/gen/proto/payment_intent/recipient"
	http "net/http"
	strings "strings"
)
...
func NewNetworkServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) NetworkServiceClient {
	...
	networkServiceMethods := recipient.File_payment_intent_recipient_recipient_proto.Services().ByName("NetworkService").Methods()
...
func NewRecipientServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) RecipientServiceClient {
	baseURL = strings.TrimRight(baseURL, "/")
	recipientServiceMethods := recipient.File_payment_intent_recipient_recipient_proto.Services().ByName("RecipientService").Methods()
